...
\begin{itemize} \itemsep +5pt
   ...
   Build directory service on Synology cloud platform (C2) from scratch, including survey and design product specification, contruct service backend on Kubernetes, and design tool to make user easy to use service.
   Designing user agent which download LDAP service data from server to end user and provide authentication service as LDAP server.
   Implementing SRP(Secure Remote Password) protocol as authentication algorithm to avoid store user password at backend to improve security of authentication server.
   \item {\bf VPN Service} Maintaining Synology VPN service application, including performance tunning make application 70\% speed up.
   \item {\bf Cross-Platform Toolchain Unify}\\
   Designing cross-platform toolchain building flow to apply security patch, avoid Synology product to suffer security attack. Also unify ARM and PowerPC platform toolchain and toolchain related issue troubleshooting.
