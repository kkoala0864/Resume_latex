...
\begin{itemize} \itemsep +5pt % reduce space between items
   ...
   Building directory service on Synology Cloud Platform (C2) from scratch.
   Construct service backend on Kubernetes.
   Design user agent which download ldap service data from server to end user and provide authentication service as LDAP server.
   Implement SRP(Secure Remote Password) protocol as authentication algorithm to avoid store user password at backend.
   \item {\bf VPN Service} Maintain Synology VPN Service Application, including performance tunning, trouble shooting.
   \item {\bf Cross-Platform Toolchain Unify}\\
   Unify ARM and PowerPC platform toolchain and toolchain related issue troubleshooting.
...
\begin{itemize} \itemsep +5pt
  ...
  Implement ARMv8 Instruction-Level accurate simulator from scratch. Design test frameword that generate test pattern and compare with golden model (ARM Fast Model) at runtime to verify simulator accuracy.
  \item {\bf ActionScript Virtual Machine}\\
  Porting Adobe AVM(ActionScript Virtual Machine) to ARM Target. Resolving issue to make AVM can execute ActionScript byte code on ARM based development board and passing regression test.
